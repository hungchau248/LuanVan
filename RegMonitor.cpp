...
int RegMonitor(LPVOID lpRegKey)
{
 ...
 DWORD WINAPI RegMon(){
	LPCSTR szRegConfig = "RegConfig.cfg";
	HANDLE hFile = CreateFile(szRegConfig, GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL );
	DWORD dwErrorCode = GetLastError();
	if(dwErrorCode == ERROR_FILE_NOT_FOUND){
		_tprintf(TEXT("Cannot open Registry Configuration file: ERROR_FILE_NOT_FOUND !\n"));
		return 1;
	}
	...
	if(dwByteRead == 0){
		_tprintf(TEXT("Registry Configuration file contains NULL !\n"));
		return 1;
	}
	...
	while(true){
		...
		if(strstr((const char *)lpBuffer, "#") != NULL){
			lpBuffer = strstr((const char *)lpBuffer, "\n") + strlen("\n");
		}
		PCHAR lpKeyStart =  strstr((const char *)lpBuffer, "<Key>");
		PCHAR lpKeyEnd =  strstr((const char *)lpBuffer, "</Key>"); 
		...
		strncpy((char *) lpKey, (const char *)lpKeyStart, (size_t)(lpKeyEnd - lpKeyStart));
		_tprintf(TEXT("Key: %s \n"), lpKey);
		...
		PCHAR lpTmp = strstr((const char *) lpKey, "\\");
		
		strncpy(lpMainKey, lpKey, (size_t)(lpTmp - lpKey));
		lpKey = lpTmp + sizeof("\\");
		_tprintf(TEXT("Main Key: %s \n\n"),lpMainKey);
